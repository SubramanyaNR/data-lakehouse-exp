...
pkill -f "port-forward.*postgresql" 2>/dev/null
...
echo "    Password: $(kubectl get secret -n postgres postgresql -o jsonpath="{.data.postgres-password}" 2>/dev/null | base64 -d)"
echo ""
echo "=========================================="
echo ""
...
while true; do
    ...
    kubectl port-forward svc/postgresql -n postgres $POSTGRES_PORT:5432 --address 0.0.0.0 2>/dev/null &
    kubectl port-forward svc/my-cluster-kafka-bootstrap -n kafka 9092:9092 --address 0.0.0.0 2>/dev/null &
    sleep 30
    ...
    pgrep -f "port-forward.*postgresql" > /dev/null || echo "[PostgreSQL] Reconnecting..."
    pgrep -f "port-forward.*kafka" > /dev/null || echo "[Kafka] Reconnecting..."
done
